...
(async () => {
  ...
  if (!folder) {
    try {
      const resp = await fetch('../Backend/api/session-start.php', {
        method: 'POST',
  ...
  fetch('../data/questions.json')
    ...
    .catch(() => alert('Unable to load questions.json'));
})();
...
function startBreak() {
  ...
  if (currentIdx === 0) {
    ...
    status.innerHTML = '<small class="text-info">3 seconds break...</small>';
    ...
    startCountdown(TIME_CONFIG.BREAK_TIME, 'break', () => {
      status.innerHTML = '<small class="text-primary">Record in (5s)...<br><strong>Press "Start now" to start</strong></small>';
      skipPrepareBtn.style.display = 'block';
      skipPrepareBtn.querySelector('button').disabled = false;
      skipPrepareBtn.querySelector('button').textContent = 'Start now';
  ...
  try {
    ...
    recorder.onstop = () => {
      ...
      if (recordedBlob.size > MAX_SIZE) {
        status.innerHTML = '<small class="text-danger">Video too big (>200MB)! Record shorter.</small>';
        recordedBlob = null;
        setTimeout(() => showQuestion(currentIdx), 2000);
        return;
      }

      status.innerHTML = '<small class="text-success">Record success!</small>';
    ...
    status.innerHTML = '<small class="text-warning">Recording... </small>';
    startCountdown(TIME_CONFIG.RECORDING_TIME, 'recording', handleRecordingComplete);

  } catch (err) {
    console.error('Camera error:', err);
    status.innerHTML = '<small class="text-danger">Can not access camera!</small>';
  }
}
...
async function uploadVideo(blob, attempt = 1) {
  ...
  try {
    ...
    uploadStatus.innerHTML = `<div class="alert alert-success">Upload success!</div>`;

    fetch('../Backend/api/transcribe.php', {
      method: 'POST',
    ...
    if (attempt <= 3) {
      ...
      uploadStatus.innerHTML = '<div class="alert alert-danger">Upload failed after 3 retries!</div>';
